...
const ProtectedRoute = ({ children }: PropsProtectedRoute) => {
  ...
  if (
    String(token.tipoUsuario) === String(typeUsers.doctor) &&
    location.pathname.includes(correspondingModule.url)
  ) {
    return children; // Permite acceso a rutas de doctores
  } else if (
    token.tipoUsuario === typeUsers.paciente &&
    location.pathname.includes(correspondingModule.url)
  ) {
    return children; // Redirige a home si es paciente
  } else if (
    token.tipoUsuario === typeUsers.administrador &&
    location.pathname.includes(correspondingModule.url)
  ) {
    return children; // Redirige a home si es paciente
  }
