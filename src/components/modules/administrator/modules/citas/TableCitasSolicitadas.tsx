              ...
              filteredCitas.map(e => {
                return <Table.Row key={e.id} className="bg-white dark:border-gray-700 dark:bg-gray-800">
                  ...
                  <Table.Cell className='flex gap-x-2'>
                    <Link to={`/administrador/programar_cita/${e.id}`}><button type="button" className="w-fit text-white bg-primary-600 hover:bg-primary-700 focus:ring-4 focus:outline-none focus:ring-primary-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-primary-600 dark:hover:bg-primary-700 dark:focus:ring-primary-800">Programar</button></Link>
                    <button 
                      type="button"
                      onClick={() => {
                        modalOpen(e.id) 
